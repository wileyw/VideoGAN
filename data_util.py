...
def get_full_clips(data_dir, hist_len, num_clips, num_rec_out=1):
    ...
    """
    clips = np.empty(
        [num_clips, FULL_HEIGHT, FULL_WIDTH, (3 * (hist_len + num_rec_out))]
    )
    ...
    for clip_num, ep_dir in enumerate(ep_dirs):
        ...
        start_index = np.random.choice(
            len(ep_frame_paths) - (hist_len + num_rec_out - 1)
        )
        clip_frame_paths = ep_frame_paths[
            start_index : start_index + (hist_len + num_rec_out)
        ]
