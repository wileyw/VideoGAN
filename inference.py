    ...
    for i in range(int(np.ceil(fw / cw))):
        for j in range(int(np.ceil(fh / ch))):
            ...
            if ((j + 1) * ch) > fh:
                y = fh - (j * ch)
            img[:x, :y, :] = images[
                i * cw : min(fw, (i + 1) * cw), j * ch : min(fh, (j + 1) * ch), :
            ]
            batch.append(img)
...
def save_to_video(frames, video_filename):
    video = cv2.VideoWriter(video_filename, -1, 1, frames[0].shape[:2])
    for frame in frames:
        video.write(frame)
...
def main():
    ...
    parser.add_argument("video_filename", help="output filename of video")
    parser.add_argument(
        "-i",
        "--input_dir",
