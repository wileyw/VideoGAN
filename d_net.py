    ...
    def __init__(
        ...
        for i in range(len(kernel_sizes)):
            self.conv_layers.append(
                nn.Conv2d(conv_layer_fms[i], conv_layer_fms[i + 1], kernel_sizes[i])
            )
        ...
        img_height /= 2
        img_width /= 2
        scale_fc_layer_sizes.insert(0, int(img_height * img_width * conv_layer_fms[-1]))
        for i in range(len(scale_fc_layer_sizes) - 1):
            self.fc_layers.append(
                nn.Linear(scale_fc_layer_sizes[i], scale_fc_layer_sizes[i + 1])
            )
...
class DiscriminatorModel(nn.Module):
    def __init__(
        ...
        self.img_dims = []
        zip_data = zip(
            kernel_sizes_list, conv_layer_fms_list, scale_fc_layer_sizes_list
        )
        for scale_num, data in enumerate(zip_data):
            kernel_sizes, conv_layer_fms, scale_fc_layer_sizes = data
            scale_factor = 1.0 / 2 ** ((len(kernel_sizes_list) - 1) - scale_num)
            img_dim = (int(img_height * scale_factor), int(img_width * scale_factor))
